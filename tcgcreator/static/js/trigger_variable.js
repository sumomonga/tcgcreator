	var trigger_kind_id = [];
	var condition_i = 0;
	function getTriggerConditionVariables(){
		$.ajax({
		   'type': "POST",
	...
	function addTriggerConditionVariables(i){
		...
		html+='<select id="variable_condition_add_'+(i+1)+'" onchange="addTriggerConditionVariable('+(i+1)+')" class="variable_condition" ><option value=""></option><option value="and">かつ</option><option value="or">または</option></select>';
